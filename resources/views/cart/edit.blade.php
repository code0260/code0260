            ...
            <div class="form-group">
                <div id="specifications-container">
                    ...
                    <div class="specification-item mb-3" data-index="{{ $index }}">
                        <button type="button" class="btn btn-info toggle-specification" data-target="#specification-{{ $index }}" data-spec-id="{{ $index }}">
                            Show {{ $specification['name'] }}  <!-- عرض اسم السبيسفكيشن هنا -->
                        </button>
                        <div class="specification-details" id="specification-{{ $index }}" style="display: none;">
                            <!-- Add Specification Fields Here -->
                            <label for="specifications[{{ $index }}][name]">Name</label>
                            <input type="text" name="specifications[{{ $index }}][name]" id="spec-name-{{ $index }}" class="form-control modern-input" value="{{ $specification['name'] }}">
...
<style>
    ...
    .modern-btn:hover {
        background-color: #333333; /* لون أغمق عند التحويم */
  ...
  /* إزالة تأثير hover على زر Show */
.toggle-specification {
    background-color: #109faf; /* اللون الأساسي */
    color: white;
    padding: 12px 20px;
    border: none;
    border-radius: 8px;
    ...
    display: inline-block;
    margin-top: 10px;
    width: 100%; /* ضمان أن يكون الأزرار بعرض 100% */
    text-align: center;
    transition: none; /* إيقاف تأثيرات التغيير عند التحويم */
}

/* عند تحريك الفأرة فوق الزر، لا تغير اللون أو الحجم */
.toggle-specification:hover {
    background-color: #109faf; /* نفس اللون كما هو في الحالة العادية */
    transform: none; /* إيقاف التأثير عند تحريك الفأرة */
}
